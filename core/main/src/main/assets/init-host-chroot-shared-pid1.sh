...
NAMESPACE_PID_FILE=$PREFIX/local/.alpine-ns-pid1
...
if [ "$NS_EXISTS" = "1" ]; then
    ...
    # These sessions will NOT be PID 1, they join the existing namespace
    su -c "nsenter -t $NS_PID -m -p -u -i chroot \"$ALPINE_DIR\" /bin/sh \"$PREFIX/local/bin/init\" \"\$@\""
else
    # Create new namespace with PID 1 init (first session)
    # This session's init becomes PID 1 and acts as the namespace daemon
    su -c "
        unshare -m -p -u -i -f --mount-proc=\"$ALPINE_DIR/proc\" sh -c '
            ...
            # This becomes the namespace daemon - when it exits, namespace is destroyed
    ...
    # First session: Try to enter the newly created namespace
    # This is needed because the background process above runs the first session
    # But we need to return control to the terminal, so we enter via nsenter
    if [ -f "$NAMESPACE_PID_FILE" ]; then
        NS_PID=$(cat "$NAMESPACE_PID_FILE" 2>/dev/null)
